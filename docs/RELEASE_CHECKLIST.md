...
5. Confirm the Blueprint has these production security variables:
   - API service `phoamthuc`: `ADMIN_API_TOKEN` is generated by Render.
   - CMS service `phoamthuc-cms`: `CMS_ADMIN_USERNAME=admin`.
   - CMS service `phoamthuc-cms`: `CMS_ADMIN_PASSWORD=admin` for demo submission.
6. For a public/long-running deployment, change `CMS_ADMIN_PASSWORD` in Render Environment after the demo.
7. Wait for the web services to finish deploying.
8. Open `https://<your-render-service>.onrender.com/health` and confirm it returns `{"status":"ok"}`.
9. Open the CMS URL and confirm protected pages redirect to `/Login`; sign in with `admin/admin` unless you changed the password.
