...
| F08 | Duyet hoac tu choi thanh toan goi app | Quan tri vien CMS | `DuyetThanhToan/Index.OnGetAsync()`, `LoadStatsAsync()`, `LoadListAsync()`, `OnGetPendingSnapshotAsync()`, `OnPostApproveAsync()`, `OnPostRejectAsync()` | `VinhKhanhTour.CMS/Pages/DuyetThanhToan/Index.cshtml*`, model `YeuCauThanhToan`, `DangKyApp` | `08-app-payment-approval-activity.puml`, `08-app-payment-approval-sequence.puml` |
| F09 | Theo doi khach hang, trang thai su dung, XP va hanh trinh | Quan tri vien CMS | `BanDo/Index.OnGetAsync()`, `LoadSubscriptionsPortableAsync()`, `LoadLocationsPortableAsync()`, `LoadPoiActivityCountsAsync()`, `BuildCustomerRows()`, `ApplySearch()`, `ApplyFilter()`, `ApplySort()` | `VinhKhanhTour.CMS/Pages/BanDo/Index.cshtml*`, `AppDbContext`, cac model lien quan | `09-live-map-activity.puml`, `09-live-map-sequence.puml` |
| F10 | Xem dashboard tong quan va monitoring CMS | Quan tri vien CMS | `Index.OnGetAsync()`, `ResolveRange()`, `LoadPoiSectionAsync()`, `LoadAnalyticsWithRetryAsync()`, `LoadAnalyticsAsync()`, `LoadAnalyticsPortableAsync()`, `LoadActivityAsync()`, `LoadGeoAsync()`, `LoadTopPoiAsync()`, `LoadRevenueAsync()`, `LoadSummaryAsync()` | `VinhKhanhTour.CMS/Pages/Index.cshtml.cs`, `VinhKhanhTour.CMS/Pages/Index.cshtml` | `10-dashboard-activity.puml`, `10-dashboard-sequence.puml` |
...
| F13 | Class diagram tong quan: page MAUI, helper app, queue/geofence helper, controller API, DbContext va model nghiep vu | `13-class-diagram.puml` |
